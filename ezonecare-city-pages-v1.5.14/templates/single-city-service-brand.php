...
/* ── v1.5.2 CITY INFO BAR — Light, professional ─── */
.ez-city-bar {
    background: #f0f4f8;
...
/* ── v1.5.2 TITLE + INTRO — Clean, no box/border ── */
.ez-title-intro-wrap {
    width: 100%;
    margin: 0 auto;
    padding: 36px 5% 8px;
...
/* v1.5.2: No box, no border — clean directory style */
.ez-intro-box {
...
// v1.5.14 FIX: Agar local_intro DB mein saved hai (custom ya default se aaya ho)
// toh Elementor content render hi mat karo — double intro avoid karo
// Agar local_intro null hai (koi bhi intro nahi) toh hi Elementor content render karo
$rendered = '';
if ( ! $local_intro ) {
...
<div class="ez-service-content">
    <?php
    // v1.5.14 FIX: local_intro hai to $rendered empty hai (upar se hi render nahi kiya)
    // Sirf tab render karo jab local_intro null ho (koi intro nahi save hua)
    if ( ! empty( $rendered ) ) {
        echo $rendered;
    }
    wp_reset_postdata();
