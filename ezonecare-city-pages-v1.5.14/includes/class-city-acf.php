...
class Ezonecare_City_ACF {
    ...
    public function register_city_fields() {
        ...
        acf_add_local_field_group(array(
            ...
            'fields' => array(
                ...
                // v1.5.6: Pickup & Drop toggle
                // Is city mein pickup/drop service available hai ya nahi
                // True  = City home page pe "Free Pickup & Drop" section dikhega
                // False = Section hide rahega (jo centers pickup nahi dete unke liye)
                array(
                    'key'          => 'field_city_pickup_available',
                    'label'        => '🚚 Pickup & Drop Available',
                    'name'         => 'city_pickup_available',
                    'type'         => 'true_false',
                    'instructions' => 'Is service center mein pickup aur drop facility available hai? ON karo toh city home page pe "Free Pickup & Drop" section dikhega.',
                    'required'     => 0,
                    'default_value'=> 0,
                    'ui'           => 1,
                ...
                // v1.5.10: Brand Logos — gallery field hataya, 6 alag image fields lagaye
                // Gallery field Elementor/Classic editor mein "Add Image" button nahi deta tha
                // Ab har logo ke liye alag image field hai — bilkul Entry Gate photo jaisa kaam karta hai
                // Laptop center → Dell, HP, Acer logos | TV center → Samsung, LG, Sony logos
                // Empty field = logo nahi dikhega | Sab cities mein media library se same image reuse
                array(
                    'key'           => 'field_city_brand_logo_1',
                    'label'         => '🏷️ Brand Logo 1',
                    'name'          => 'city_brand_logo_1',
                    'type'          => 'image',
                    'instructions'  => 'Brand 1 ka logo. PNG transparent background recommended. Media library se select karo — ek baar upload, sab cities mein reuse.',
                    'required'      => 0,
                    'return_format' => 'array',
                ...
                array(
                    'key'           => 'field_city_brand_logo_2',
                    'label'         => '🏷️ Brand Logo 2',
                    'name'          => 'city_brand_logo_2',
                    'type'          => 'image',
                    'instructions'  => 'Brand 2 ka logo.',
                    'required'      => 0,
                    'return_format' => 'array',
                    'preview_size'  => 'thumbnail',
                    'library'       => 'all',
                ...
                array(
                    'key'           => 'field_city_brand_logo_3',
                    'label'         => '🏷️ Brand Logo 3',
                    'name'          => 'city_brand_logo_3',
                    'type'          => 'image',
                    'instructions'  => 'Brand 3 ka logo.',
                    'required'      => 0,
                    'return_format' => 'array',
                    'preview_size'  => 'thumbnail',
                    'library'       => 'all',
                ...
                array(
                    'key'           => 'field_city_brand_logo_4',
                    'label'         => '🏷️ Brand Logo 4',
                    'name'          => 'city_brand_logo_4',
                    'type'          => 'image',
                    'instructions'  => 'Brand 4 ka logo.',
                    'required'      => 0,
                    'return_format' => 'array',
                    'preview_size'  => 'thumbnail',
                    'library'       => 'all',
                ...
                array(
                    'key'           => 'field_city_brand_logo_5',
                    'label'         => '🏷️ Brand Logo 5',
                    'name'          => 'city_brand_logo_5',
                    'type'          => 'image',
                    'instructions'  => 'Brand 5 ka logo.',
                    'required'      => 0,
                    'return_format' => 'array',
                    'preview_size'  => 'thumbnail',
                    'library'       => 'all',
                ...
                array(
                    'key'           => 'field_city_brand_logo_6',
                    'label'         => '🏷️ Brand Logo 6',
                    'name'          => 'city_brand_logo_6',
                    'type'          => 'image',
                    'instructions'  => 'Brand 6 ka logo. (Optional — sirf fill karo agar 6 brands hain)',
                    'required'      => 0,
                    'return_format' => 'array',
                    'preview_size'  => 'thumbnail',
                    'library'       => 'all',
               ...
               array(
                ...
                // v1.5.8: Local Intro tab alag dynamic group mein hai
                // register_dynamic_local_intro_fields() se auto-generate hota hai
    ...
    public function render_reset_button( $field ) {
        ...
        // v1.5.14: Update button — sirf yeh ek field AJAX se save karo
        echo '<button type="button"
                class="button button-primary ez-update-intro-btn"
                data-city="' . esc_attr( $city_id ) . '"
                data-slug="' . esc_attr( $service_slug ) . '"
                data-field="' . esc_attr( $field['key'] ) . '"
                title="Sirf is field ko save karo — poora page save karne ki zaroorat nahi">';
        echo '💾 Update';
        echo '</button>';

        // Reset button — sirf tab dikhao jab customized ho
        $reset_style = ( $state === 'customized' ) ? '' : ' style="display:none;"';
        echo '<button type="button"
    ...
    public function enqueue_intro_scripts( $hook ) {
        ...
        wp_add_inline_style( 'wp-admin', '
            ...
            .ez-update-intro-btn:disabled {
                opacity: 0.6;
                cursor: not-allowed;
            }
        ' );

        // Inline JS — AJAX update + reset handlers (v1.5.14)
        $nonce = wp_create_nonce( 'ez_reset_intro_nonce' );
        wp_add_inline_script( 'jquery', '
        jQuery(document).ready(function($) {

            // ── UPDATE BUTTON — sirf yeh ek field AJAX se save karo ──
            $(document).on("click", ".ez-update-intro-btn", function() {
                var $btn      = $(this);
                ...
                // Textarea value lo
                var $textarea = $("textarea[data-key=\'" + field_key + "\'],#acf-" + field_key);
                if ( ! $textarea.length ) {
                    $textarea = $("textarea[name*=\'" + slug + "\']");
                }
                var intro_text = $textarea.val() || "";

                $btn.prop("disabled", true).text("Saving...");

                $.post(ajaxurl, {
                    action:       "ez_update_intro",
                    nonce:        "' . $nonce . '",
                    city_id:      city_id,
                    service_slug: slug,
                    ...
                    if ( response.success ) {
                        ...
                        $btn.closest(".ez-intro-actions")
                            .find(".ez-intro-badge")
                            .removeClass("ez-badge-default")
                            .addClass("ez-badge-custom")
                            .text("✅ Customized");

                        // Reset button dikhao
                        $btn.closest(".ez-intro-actions")
                            .find(".ez-reset-intro-btn")
                            .show();

                        // Button text momentarily Saved! dikhao
                        $btn.text("✅ Saved!");
                        setTimeout(function() {
    ...
    public static function register_load_value_hooks() {
        // v1.5.8: Fully dynamic — ek universal filter
        // Kisi bhi city_local_intro_ field ke liye kaam karta hai
        // Hardcoded slug list HATAYA — koi bhi brand assign karo, default text auto-load
        add_filter(
            'acf/load_value',
            function( $value, $post_id, $field ) {
                if ( empty( $field['name'] ) ) return $value;
                if ( strpos( $field['name'], 'city_local_intro_' ) !== 0 ) return $value;
                if ( ! empty( $value ) && trim( $value ) !== '' ) return $value;
                $slug = str_replace( 'city_local_intro_', '', $field['name'] );
                if ( empty( $slug ) ) return $value;
                return Ezonecare_City_ACF::get_service_raw_intro( $slug );
            },
            10, 3
        );
    }
    ...
    /**
     * v1.5.8 — Fully Dynamic Local Intro Fields
     * City ke "Service & Brand Assignment" meta box mein jo brands hain
     * unke liye AUTOMATICALLY ACF fields generate karta hai
     *
     * Koi bhi brand assign karo — field apne aap ban jaata hai
     * Koi bhi slug hardcoded nahi — Dell, Acer, HP, Lenovo sab ek hi tarah
     */
    public static function register_dynamic_local_intro_fields() {
        if ( ! function_exists( 'acf_add_local_field_group' ) ) return;

        // v1.5.8 CORRECT APPROACH:
        // acf_add_local_field_group() directly use karo — bilkul wahi tarika
        // jo Dell+Acer ke liye v1.5.7 mein kaam karta tha
        // acf/load_fields filter Gutenberg REST API mein unreliable hai
        // Direct registration = Gutenberg + Classic Editor dono mein kaam karta hai

        // v1.5.14 FIX: City ID multiple sources se lo — Gutenberg REST API bhi cover karo
        // Pehle $_GET['post'] (classic editor, normal page load)
        // Phir $_POST['post_ID'] (classic editor save)
        // Phir $_POST['post_id'] (Gutenberg REST API save — lowercase)
        // Phir $_GET['post_id'] (kuch REST calls mein)
        // Phir WP global $post object (REST API context mein)
        $city_id = 0;

        if ( isset( $_GET['post'] ) && intval( $_GET['post'] ) > 0 ) {
            $city_id = intval( $_GET['post'] );
        } elseif ( isset( $_POST['post_ID'] ) && intval( $_POST['post_ID'] ) > 0 ) {
            $city_id = intval( $_POST['post_ID'] );
        } elseif ( isset( $_POST['post_id'] ) && intval( $_POST['post_id'] ) > 0 ) {
            $city_id = intval( $_POST['post_id'] );
        } elseif ( isset( $_GET['post_id'] ) && intval( $_GET['post_id'] ) > 0 ) {
            $city_id = intval( $_GET['post_id'] );
        }

        // Gutenberg REST API: URL mein post ID hota hai /wp/v2/posts/123
        if ( ! $city_id && defined( 'REST_REQUEST' ) && REST_REQUEST ) {
            // REQUEST_URI se post ID nikalo
            if ( preg_match( '#/(?:posts|city)/(\d+)#', $_SERVER['REQUEST_URI'] ?? '', $m ) ) {
                $city_id = intval( $m[1] );
            }
        }

        // Last fallback: global $post object
        if ( ! $city_id ) {
            global $post;
        ...
        if ( ! $city_id ) return;
        if ( get_post_type( $city_id ) !== 'city' ) return;

        // City ke assigned brands lo
        $raw = get_post_meta( $city_id, '_ezonecare_city_service_brands', true );
        if ( empty( $raw ) ) return;
        $data = json_decode( $raw, true );
        if ( ! is_array( $data ) || empty( $data ) ) return;

        // Sabhi assigned brand post IDs collect karo
        $brand_post_ids = array();
        foreach ( $data as $svc_id => $brand_ids ) {
            if ( is_array( $brand_ids ) ) {
                foreach ( $brand_ids as $bid ) {
                    $brand_post_ids[] = intval( $bid );
                }
            }
        }
        $brand_post_ids = array_unique( array_filter( $brand_post_ids ) );
        if ( empty( $brand_post_ids ) ) return;

        // Har brand ke liye ACF field build karo
        $fields_arr = array();
        foreach ( $brand_post_ids as $bpid ) {
            $bp = get_post( $bpid );
            if ( ! $bp || $bp->post_status !== 'publish' ) continue;
            if ( $bp->post_type !== 'service' ) continue;

            $slug  = $bp->post_name;
            $title = $bp->post_title;

            $icon = '🔧';
            if ( stripos( $title, 'battery' )     !== false ) $icon = '🔋';
            if ( stripos( $title, 'keyboard' )    !== false ) $icon = '⌨️';
            if ( stripos( $title, 'screen' )      !== false ) $icon = '🖥️';
            if ( stripos( $title, 'motherboard' ) !== false ) $icon = '🔌';
            if ( stripos( $title, 'charging' )    !== false ) $icon = '⚡';
            if ( stripos( $title, 'hinge' )       !== false ) $icon = '🔩';
            if ( stripos( $title, 'data' )        !== false ) $icon = '💾';

            $fields_arr[] = array(
                'key'          => 'field_dyn_li_' . md5( $slug ),
                'label'        => $icon . ' ' . $title . ' — Local Intro',
                'name'         => 'city_local_intro_' . $slug,
                'type'         => 'textarea',
                'instructions' => '80-120 words. Placeholders: {city} {center} {area} {landmark}',
                'rows'         => 4,
                'placeholder'  => '{center} {city} mein ' . $title . ' ke liye...',
                'required'     => 0,
            );
        }
        ...
        // Sort alphabetically
        usort( $fields_arr, function( $a, $b ) {
            return strcmp( $a['label'], $b['label'] );
        });

        // Info message field bhi add karo pehle
        $all_fields = array(
            array(
                'key'     => 'field_city_tab_local_intro_dyn',
                'label'   => '📝 Local Intro',
                'type'    => 'tab',
                'placement' => 'top',
            ),
            array(
                'key'     => 'field_city_local_intro_info_dyn',
                'label'   => '📌 Local Intro — Dynamic (v1.5.8)',
                'name'    => '',
                'type'    => 'message',
                'message' => '<strong>✅ Dynamic System Active!</strong><br><br>
Ye fields city ke "Service & Brand Assignment" se automatically bante hain.<br>
Nayi brand assign karo → Save karo → Page reload karo → Field yahan aayega.<br><br>
Placeholders: <code>{city}</code> <code>{center}</code> <code>{area}</code> <code>{landmark}</code><br>
<strong>Empty</strong> = Service post se default text | <strong>Filled</strong> = Aapka custom text ✅',
            ),
        );
        ...
        // DIRECT registration — bilkul v1.5.7 Dell+Acer wali tarah
        acf_add_local_field_group( array(
            'key'      => 'group_ezonecare_city_local_intro_' . $city_id,
            'title'    => 'City Local Intro — Dynamic',
            'fields'   => $all_fields,
            'location' => array(
                array(
                    array(
    ...
    /**
     * v1.5.14: AJAX — Sirf ek intro field save karo
     * Update button click → is field ka text DB mein save karo
     * Poora city page save karne ki zaroorat nahi
     */
    public static function ajax_update_intro() {
        // Security check
        ...
        $text    = sanitize_textarea_field( wp_unslash( $_POST['intro_text'] ?? '' ) );
        ...
        update_post_meta( $city_id, $meta_key, $text );
